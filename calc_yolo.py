...
# === SETTINGS ===
CSV_FILE = "output/yolo_joints.csv"   # <-- YOLO joints CSV
OUTPUT_FILE = "output/yolo_distances.csv"
BASKET_HEIGHT_CM = 7.5        # Real-world basket height in cm
...
print("\nAverage Measurements (compared to ground truth):")
print("-" * 50)
for j1, j2 in PAIRS[:4]:  # First 4 pairs are arm measurements
    col = f"{j1}_to_{j2}_cm"
    mean_val = results_df[col].mean()
    if "shoulder" in j1 and "elbow" in j2:
        truth = GROUND_TRUTH["shoulder_to_elbow"]
    else:
        truth = GROUND_TRUTH["elbow_to_wrist"]
    error = abs(mean_val - truth)
    print(f"{col:<35}: {mean_val:.2f} cm (error: {error:.2f} cm)")
